...
// template automatically gets a fresh, unchecked instance added on each of
// its selected days, every week — see ensureWeekChores() in routes/chores.js.
router.get('/', (req, res) => {
...
router.post('/', (req, res) => {
  ...
  const info = db
    .prepare('INSERT INTO chore_templates (title, assigned_to, days, sort_order) VALUES (?, ?, ?, ?)')
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE chore_templates SET title=@title, assigned_to=@assigned_to, active=@active, days=@days WHERE id=@id').run({
    ...merged,
    id: req.params.id,
  });
